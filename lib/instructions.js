...
function blockFor(agentLabel, memoryDir) {
  ...
  return [
    ...
    '',
    `- **Task start:** call \`${prefix}search_nodes\` with 1-3 keywords (file/feature names) to load prior context. **Never \`read_graph\`** (dumps everything). Skip for trivial/conversational asks.`,
    `- **Task end, or when I say "remember X":** save ONE telegraphic line via \`${prefix}add_observations\` (or \`${prefix}create_entities\` for a new file/feature). Format: \`file/feature: what changed (${agentLabel}, YYYY-MM-DD HH:MM)\` in local 24h time — check the clock if unsure. No prose. Save once per unit of work, not per edit. If the entity exists, add to it — don't duplicate.`,
    '',
    `Details: ${readme}`,
    END,
    '',
